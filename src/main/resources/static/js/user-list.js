...
$(function(){
    ...
    $(grid_selector).jqGrid({
        ...
        colModel: [
            ...
            { label: '操作', name: 'btn', width: 50, formatter: function (cellvalue, options, cell) {
                return '<button  userid='+cell.id+' username="'+cell.username+'" class="btnResetPwd btn btn-purple btn-sm"><i class="fa fa-cog fa-spin" aria-hidden="true"></i>重置密码</button>';
            }},
        ],
    ...
    $(document).on('click', ".btnResetPwd", function(){
        ...
        $.messager.confirm("温馨提示", "是否确定重置用户["+username+"]的密码？", function() {
            $.ajax({
                ...
                data:{"id": id},
                beforeSend : function(){
                    loading=layer.load("正在重置中...");
                },
                success:function(result){
                    if(result.isSuccess){
                        if(result.isSelf){
                            alert("您重置了自己的密码，请重新登录");
                            top.location.reload();
                        }else{
                            layer.msg('重置用户['+username+']的密码成功！', {icon: 7,time: 2000});
                        }
                    }else{
                        layer.msg('重置失败！', {icon: 7,time: 2000});
                    }
                },error:function(){
